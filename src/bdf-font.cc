...
int Font::DrawGlyph(BitCanvas *c, int x_pos, int y_pos, bool inverse,
...
int DrawText(BitCanvas *c, const Font &font,
             int x, int y,
             const char *utf8_text, bool inverse, int extra_spacing) {
  const int start_x = x;
  while (*utf8_text) {
    const uint32_t cp = utf8_next_codepoint(utf8_text);
    x += font.DrawGlyph(c, x, y, inverse, cp);
    x += extra_spacing;
  }
  return x - start_x;
}
